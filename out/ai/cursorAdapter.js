...
exports.CursorAIAdapter = void 0;
 ...
 * 4. Fallback 到手动方式
 */
class CursorAIAdapter {
    constructor(logger, chatCommandId) {
        this.logger = logger;
        this.chatCommandId = chatCommandId;
    }
    /**
     * 分析代码
     */
     ...
     */
    async tryLanguageModelAPI(prompt, token) {
        // 检查 API 是否可用
        if (!vscode.lm) {
     ...
     */
    async tryCursorComposer(prompt, token) {
        // 检查 Composer 命令是否可用
     ...
     * 尝试使用 Cursor Chat 命令
     */
    async tryCursorChatCommand(prompt, token) {
        if (!this.chatCommandId) {
            return null;
        }
        try {
            // 尝试执行 Chat 命令并传递 prompt
            const result = await vscode.commands.executeCommand(this.chatCommandId, { message: prompt, silent: true });
            if (result && typeof result === 'string') {
                return result;
            }
        }
     ...
     * 这种方式用户体验更好，不需要来回复制
     */
    async inlineAssistMode(prompt, token) {
        ...
        // 选中全部内容
        const fullRange = new vscode.Range(tempDoc.positionAt(0), tempDoc.positionAt(tempDoc.getText().length));
        editor.selection = new vscode.Selection(fullRange.start, fullRange.end);
        // 提示用户操作
        const choice = await vscode.window.showInformationMessage('🤖 请使用 Cursor AI 分析代码', {
            modal: true,
            detail: '操作步骤：\n\n' +
     ...
     */
    async waitForManualResponse(originalPrompt, token) {
        const result = await vscode.window.showInformationMessage('📋 Prompt 已复制到剪贴板', {
            modal: true,
        ...
        if (result === '🔄 重新复制') {
            ...
            if (!response || response === originalPrompt || response.length < 50) {
                const retry = await vscode.window.showWarningMessage('⚠️ 剪贴板内容似乎不是有效的 AI 分析结果', '重试', '强制使用', '取消');
                if (retry === '重试') {
                    return this.waitForManualResponse(originalPrompt, token);
                }
                else if (retry === '强制使用') {
...
exports.CursorAIAdapter = CursorAIAdapter;
