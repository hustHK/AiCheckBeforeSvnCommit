...
exports.AIProviderSelector = void 0;
...
class AIProviderSelector {
     ...
     * 显示 AI 提供商选择界面
     */
    async showProviderSelection(isCursorEnv) {
     ...
     * 确保 API Key 已配置
     */
    async ensureApiKeyConfigured(provider) {
        // 检查是否已有 API Key
        let apiKey = '';
        switch (provider.id) {
            case 'deepseek':
        ...
        if (apiKey) {
            // 已有 API Key，询问是否使用
            const choice = await vscode.window.showInformationMessage(`检测到已配置的 ${provider.label} API Key`, '使用现有配置', '重新配置');
            if (choice === '使用现有配置') {
                return true;
            }
        }
     ...
     * 显示 API Key 配置向导
     */
    async showApiKeyConfiguration(provider) {
        // 显示帮助信息
        const helpUrls = {
        ...
        const choice = await vscode.window.showInformationMessage(message, { modal: true }, '打开网页', '已有 API Key', '取消');
        if (choice === '取消') {
            return false;
        }
        if (choice === '打开网页') {
        ...
        vscode.window.showInformationMessage(`✅ ${provider.label} API Key 配置成功！`);
        return true;
    }
    /**
     * 显示模型选择（可选）
...
exports.AIProviderSelector = AIProviderSelector;
AIProviderSelector.PROVIDERS = [
    {
        id: 'cursor',
...
//# sourceMappingURL=aiProviderSelector.js.map