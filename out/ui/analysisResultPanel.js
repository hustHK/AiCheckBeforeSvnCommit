...
class AnalysisResultPanel {
    constructor(panel, extensionUri, logger) {
        this.logger = logger;
        this.disposables = [];
        this.panel = panel;
     ...
     * 创建或显示分析结果面板
     */
    static createOrShow(extensionUri, logger) {
        ...
        // 否则创建新面板
        const panel = vscode.window.createWebviewPanel(AnalysisResultPanel.viewType, '🔍 AI Code Review', column || vscode.ViewColumn.Two, {
            enableScripts: true,
     ...
     * 显示加载状态
     */
    showLoading(message = '正在分析代码...') {
        this.panel.webview.html = this.getLoadingHtml(message);
    }
    /**
     * 显示分析结果
     */
    showResult(markdownContent, metadata) {
...
<body>
     ...
     * 获取结果页面 HTML
     */
    getResultHtml(markdownContent, metadata) {
...
AnalysisResultPanel.viewType = 'svnAIAnalysisResult';
//# sourceMappingURL=analysisResultPanel.js.map