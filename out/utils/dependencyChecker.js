...
class DependencyChecker {
     ...
     */
    async checkAndHandleDependencies() {
     ...
     * 处理缺失的必需依赖
     */
    async handleMissingRequired(missing) {
        ...
        if (choice === '立即安装') {
            ...
            if (allInstalled) {
                // 提示用户重新加载窗口
                const reload = await vscode.window.showInformationMessage('✅ 依赖扩展安装完成！需要重新加载窗口以激活。', '重新加载', '稍后');
                if (reload === '重新加载') {
                ...
                vscode.window.showErrorMessage('部分扩展安装失败，请尝试手动安装。');
                return false;
            }
        }
        else if (choice === '手动安装') {
            ...
            for (const dep of missing) {
                await vscode.commands.executeCommand('workbench.extensions.search', dep.id);
            }
            vscode.window.showInformationMessage('请在扩展面板中搜索并安装所需扩展，然后重新加载窗口。');
            return false;
        }
        else {
            // 用户选择暂不安装
            this.logger.warn('User chose not to install required dependencies');
            vscode.window.showWarningMessage('SVN Commit AI Check 将在功能受限模式下运行。某些功能可能不可用。');
            return false;
        }
    }
    /**
     * 处理缺失的可选依赖
     */
    async handleMissingOptional(missing) {
        // 可选依赖只显示一次提示，不阻塞启动
        const extensionNames = missing.map(d => d.displayName).join(', ');
        ...
        const choice = await vscode.window.showInformationMessage(`推荐安装以下扩展以获得更好的体验：${extensionNames}`, '安装', '不再提示');
        if (choice === '安装') {
    ...
    async installExtensions(extensions) {
        ...
        for (const ext of extensions) {
            try {
                ...
                allSuccess = false;
                // 显示错误并提供备选方案
                const retry = await vscode.window.showErrorMessage(`安装 ${ext.displayName} 失败`, '重试', '打开扩展市场', '跳过');
                if (retry === '重试') {
                    // 递归重试
     ...
     * 获取依赖状态摘要
     */
    async getDependencyStatusSummary() {
        const result = await this.checkDependencies();
        let summary = '=== 扩展依赖状态 ===\n';
        for (const dep of DependencyChecker.DEPENDENCIES) {
